...
class SignupRepository(
    ...
    fun setEventListener(listener: SignupEventListener) { signupEventListener = listener }

    suspend fun signUp(username: String, password: String) {
        signupUsecase.call(
            SignupParams(request = SignupParams.Request(username = username, password = password)),
                    ...
                    when (response) {
                        is IResponse.Success -> {
                            response.data.data?.let {
                                context.launch { loginAfterSignup(username, password) }
                            }
                        }
    ...
    private suspend fun loginAfterSignup(username: String, password: String) {
        loginUsecase.call(
            LoginParams(request = LoginParams.Request(username = username, password = password)),
            object : IResponseHandler<ParentResponse<LoginResponse>, IResponse<ParentResponse<LoginResponse>>> {
                    ...
                    when (response) {
                        is IResponse.Success -> {
                            response.data.data?.let {
                                ...
                                context.launch { signupEventListener?.onSignUpAndLoginSuccess() }
                            }
                        }
                        is IResponse.Failure -> context.launch { signupEventListener?.onSignUp() }
                        is Response -> {}
                    }
                }
            }
